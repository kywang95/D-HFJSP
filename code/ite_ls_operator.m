function [endvalue,endsolution,ls_trend,nfesa]=ite_ls_operator(t0SA,startvalue,startsolution,data,bestsf,method)
...
nfesa = 0;
iteration = size(data.Specimen, 2);
%   iteration = max(data.Specimen(3,:));
% for i=1:dim*(dim-1)
while nfesa < ceil(iteration/10)
    switch method
        case 1
